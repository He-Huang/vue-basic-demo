  ...
  <div class="home">
    ...
    <div v-for="todo in todos" class="todo-item" :class="{'is-completed':todo.completed}">
      <p>
        ...
        {{todo.id}}--{{todo.title}}
        <input type="button" value="X" @click="deleteItem(todo.id)" class="del" />
        <input type="button" value="editor" @click="putItem(todo.id)" class="edit">
      </p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    putItem(id){
      console.log(id);
      axios.put(`http://jsonplaceholder.typicode.com/todos/${id}`).then(
        res=>{
